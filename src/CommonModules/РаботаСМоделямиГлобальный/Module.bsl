...
//  ИменаРезультатов	- Строка - имена реквизитов через запятую, которые устанавливает обработчик
//  ИменаПараметров		- Строка - имена реквизитов и свойств через запятую, от которых зависит обработчик
//  СтруктураПараметры	- Структура - значения параметров обработчика
//						- Булево - если Истина, то параметры необходимо заполнять внутри вызова глМодель_НачатьУстановкуПараметровОбработчика
// 
// Возвращаемое значение:
//  Булево - 
//
Функция глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРезультатов, ИменаПараметров, СтруктураПараметры = Ложь) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	...
	Иначе
		...
		Если ТипЗнч(СтруктураПараметры) = Тип("Структура") Тогда
			Для Каждого ЭлементСтруктуры Из СтруктураПараметры Цикл
		...
		Если СтруктураПараметры = Истина Тогда
		...
		Если ПараметрыОбработчика.Свойство("_ТребуетсяВызовСервера_")
			И ПараметрыОбработчика["_ТребуетсяВызовСервера_"] Тогда
		...
		Возврат СтруктураПараметры = Истина;
	КонецЕсли;
КонецФункции
...
//							* "_ТребуетсяВызовСервера_" - для выполнения обработчика требуется вызов сервера
//  Значение			 - 	 - 
//
Процедура глМодель_УстановитьПараметрОбработчика(КонтекстИзменений, ИмяПараметра, Значение) Экспорт
	...
	Если ИмяПараметра = "_ТребуетсяВызовСервера_"
