...
//						 - Структура - еси изменяется несколько реквизитов, в этом случае ДанныеСтрокиНовые не используется
//							* Ключ - имя таблицы
...
Функция глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРезультатов, ИменаПараметров, Параметры = Ложь) Экспорт
	...
	Иначе
		...
		Для Каждого ИмяРезультата Из МассивИменаРезультата Цикл
			...
			ВставитьСвойство(СтруктураОбработчиков.ИзмененныеРеквизиты, ИмяРезультата, Истина, ДействияОтмены);
		КонецЦикла;
