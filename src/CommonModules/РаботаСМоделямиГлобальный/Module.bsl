...
Функция глМодель_ЕстьРеквизиты(Данные, ИменаРеквизитов) Экспорт
	...
	СтруктураЗначения = Новый Структура(ИменаРеквизитов);
	Для Каждого ИмяРеквизита Из МассивИменаРеквизитов Цикл
		СтруктураЗначения[ИмяРеквизита] = Null;
	КонецЦикла;
...
Процедура глМодель_ОбновитьСвойства(КонтекстИзменений, ИменаСвойств = Неопределено) Экспорт
	...
	КонтекстИзменений.Действия.Вставить("Модель_ОбновитьСвойства", ИменаСвойств);
	глМодель_ОбработатьДействия(КонтекстИзменений);
...
Процедура глМодель_ОбновитьСвойстваСтроки(КонтекстИзменений, ДанныеСтроки = Неопределено, ИменаСвойств = Неопределено) Экспорт
	...
	ТекущиеДействия = КонтекстИзменений.Действия;
	КонтекстИзменений.Действия = Новый Структура;
	КонтекстИзменений.Действия.Вставить("Модель_ОбновитьСвойстваСтроки", Новый Структура("ДанныеСтроки,ИменаСвойств", ДанныеСтроки, ИменаСвойств));
	глМодель_ОбработатьДействия(КонтекстИзменений);
	КонтекстИзменений.Действия = ТекущиеДействия;
	#Иначе
	ОбъектФорма = КонтекстИзменений.ОбъектФорма;
...
Функция глМодель_ВыполнятьОбработчик(КонтекстИзменений, ИменаРезультата, ИменаПараметров) Экспорт
	Возврат глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, Неопределено, ИменаРезультата, ИменаПараметров);
КонецФункции

Функция глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРезультата, ИменаПараметров) Экспорт
	Если ДанныеСтроки = Неопределено Тогда
		СтруктураОбработчиков = КонтекстИзменений.Обработчики;
		Данные = глМодель_Данные(КонтекстИзменений);
	Иначе
	...
	Если КонтекстИзменений.ВыполнятьОбработчики Тогда
		...
		Для Каждого ИмяПараметра Из ПолучитьКлючи(ИменаПараметров) Цикл
			Если СтруктураОбработчиков.Результаты[ИмяПараметра] <> Неопределено Тогда
				Возврат Ложь;
			КонецЕсли;
			
			Если СтруктураОбработчиков.Свойства[ИмяПараметра] <> Неопределено Тогда
				СтруктураОбработчиков.СвойстваКОбновлению.Вставить(ИмяПараметра);
				Возврат Ложь;
			КонецЕсли;
		КонецЦикла;
...
Функция глМодель_ОбработатьДействия(КонтекстИзменений, Форма = Неопределено) Экспорт
	...
	Если КонтекстИзменений.Действия.Количество() > 0 Тогда
		...
		Если КонтекстИзменений.Действия.Количество() = 1
			И КонтекстИзменений.Действия.Свойство("Модель_ОбновитьСвойстваСтроки") Тогда
			
			Структура = КонтекстИзменений.Действия.Модель_ОбновитьСвойстваСтроки;
			Если ТипЗнч(Структура.ДанныеСтроки) <> Тип("Строка") Тогда
				ДанныеСтроки = Структура.ДанныеСтроки;
				Структура.ДанныеСтроки = глМодель_ПолучитьИмяТаблицыСтроки(КонтекстИзменений, ДанныеСтроки);
				Структура.Вставить("ИдентификаторСтроки", ДанныеСтроки.ПолучитьИдентификатор());
			КонецЕсли;
		КонецЕсли;
		...
		#Иначе
		Если КонтекстИзменений.Действия.Количество() = 1
			И КонтекстИзменений.Действия.Свойство("Модель_ОбновитьСвойства") Тогда
			
			глМодель_ОбновитьСвойства(КонтекстИзменений, КонтекстИзменений.Действия.Модель_МодельОбновитьСвойства);
			
		ИначеЕсли КонтекстИзменений.Действия.Количество() = 1
			И КонтекстИзменений.Действия.Свойство("Модель_ОбновитьСвойстваСтроки") Тогда
			
			ПараметрыДействия = КонтекстИзменений.Действия.Модель_ОбновитьСвойстваСтроки;
			глМодель_ОбновитьСвойстваСтроки(КонтекстИзменений
				,?(ПараметрыДействия.Свойство("ИдентификаторСтроки")
					,глМодель_Данные(КонтекстИзменений)[ПараметрыДействия.ДанныеСтроки].НайтиПоИдентификатору(ПараметрыДействия.ИдентификаторСтроки)
					,ПараметрыДействия.ДанныеСтроки)
				,ПараметрыДействия.ИменаСвойств);
		Иначе
			Если ТипЗнч(ТекущийОбъектФорма) = Тип("УправляемаяФорма") Тогда
				ИмяОбъекта = ТекущийОбъектФорма.ИмяФормы;
			Иначе
				ИмяОбъекта = ТекущийОбъектФорма.Метаданные().ПолноеИмя();
			КонецЕсли;
...
Процедура ОбновитьСвойства(КонтекстИзменений, ДанныеСтроки, СтруктураОбработчиков, ОкончательныйРасчет = Ложь)
	...
	Для Каждого ДанныеСвойства Из СтруктураОбработчиков.Свойства Цикл
		...
		Для Каждого ИмяПараметра Из ДанныеСвойства.Значение Цикл
			Если СтруктураОбработчиков.Результаты[ИмяПараметра] <> Неопределено
				ИЛИ СтруктураОбработчиков.Свойства[ИмяПараметра] <> Неопределено Тогда
