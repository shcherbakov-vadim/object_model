...
//						- СтрокаКоллекции - 
//  Кэш					 - 	 - 
//  ДанныеИсходнойСтроки - Структура - значение будет записано в свойство контекста ДанныеИсходнойСтроки
// 
// Возвращаемое значение:
//   СтрокаКоллекции - 
//
Функция глМодель_ДобавитьСтроку(КонтекстИзменений, СтрокаИмяКоллекции, Кэш = Неопределено, ДанныеИсходнойСтроки = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш, ДанныеИсходнойСтроки);
		НоваяСтрока = глМодель_ДобавитьСтроку(НовыйКонтекст, СтрокаИмяКоллекции, ДанныеИсходнойСтроки);
		глМодель_ОбработатьДействия(НовыйКонтекст);
		Кэш = ДанныеКонтекста(НовыйКонтекст).Кэш;
		Возврат НоваяСтрока;
	КонецЕсли;
...
//  ДанныеИсходнойСтроки - Структура - значение будет записано в свойство контекста ДанныеИсходнойСтроки
//
Процедура глМодель_ПриДобавленииСтроки(КонтекстИзменений, ДанныеСтроки, Кэш = Неопределено, ДанныеИсходнойСтроки = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш, ДанныеИсходнойСтроки);
...
Функция глМодель_ПолучитьСвойствоКонтекста(КонтекстИзменений, ИмяПараметра) Экспорт
	Если ИмяПараметра = "Кэш"
		ИЛИ ИмяПараметра = "ДанныеИсходнойСтроки" Тогда
