...
//	Кэш -
//
Процедура глМодель_ИзменитьРеквизит(КонтекстИзменений, ИмяРеквизита, Значение, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
		глМодель_ИзменитьРеквизит(НовыйКонтекст, ИмяРеквизита, Значение);
		глМодель_ОбработатьДействия(НовыйКонтекст);
		Кэш = ДанныеКонтекста(НовыйКонтекст).Кэш;
		Возврат;
	КонецЕсли;
...
//	Кэш -
//
Процедура глМодель_ИзменитьРеквизиты(КонтекстИзменений, СтруктураЗначения, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
...
//  ДанныеСтрокиСтарые	 - Структура - если изменяется только один реквизит, значения реквизитов строки таблицы до редактирования
//						 - Структура - если изменяется несколько реквизитов, в этом случае ДанныеСтрокиНовые не используется
//							* Ключ - имя таблицы
//							* Значение - измененные строки таблицы
//						 - Соответствие - измененные строки таблицы, в этом случае ДанныеСтрокиНовые не используется
//							* Ключ - данные удаленной или новой строки
//							* Значение - Структура - старые значения реквизитов
//										
//  ДанныеСтрокиНовые	 - СтрокаКоллеции - новые данные строки
//	Кэш -
//
Процедура глМодель_ПриИзмененииРеквизитов(КонтекстИзменений, ИменаРеквизитов, ДанныеСтрокиСтарые = Неопределено, ДанныеСтрокиНовые = Неопределено, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
	...
	Иначе
		...
		Для Каждого ИмяРеквизита Из МассивИменаРеквизитов Цикл
			...
			Если ДанныеСтрокиСтарые = Неопределено
				...
				Если ТипЗнч(ИменаРеквизитов) = Тип("Строка")
					ИЛИ ТипЗнч(ИменаРеквизитов) = Тип("Массив")
					ИЛИ ТипЗнч(ИменаРеквизитов) = Тип("ФиксированныйМассив") Тогда
					
					СтруктураОбработчиков.ИзмененныеРеквизиты.Вставить(ИмяРеквизита, Null);
				Иначе
					СтруктураОбработчиков.ИзмененныеРеквизиты.Вставить(ИмяРеквизита, ИменаРеквизитов[ИмяРеквизита]);
				КонецЕсли;
			Иначе
				Если МассивИменаРеквизитов.Количество() > 1 Тогда
					Если ДанныеСтрокиСтарые.Свойство(ИмяРеквизита) Тогда
						СтруктураОбработчиков.ИзмененныеРеквизиты.Вставить(ИмяРеквизита, ДанныеСтрокиСтарые[ИмяРеквизита]);
					КонецЕсли;
				Иначе
					Если ТипЗнч(ДанныеСтрокиСтарые) = Тип("Соответствие") Тогда
						СтруктураОбработчиков.ИзмененныеРеквизиты.Вставить(ИмяРеквизита, ДанныеСтрокиСтарые);
					Иначе
						...
						Если ДанныеСтрокиНовые <> Неопределено Тогда
							Если ДанныеСтрокиСтарые <> Неопределено Тогда
								СоответствиеСтроки.Вставить(ДанныеСтрокиНовые, ДанныеСтрокиСтарые);
							Иначе
								СоответствиеСтроки.Вставить(ДанныеСтрокиНовые, Новый Структура);
							КонецЕсли;
						Иначе
							СоответствиеСтроки.Вставить(ДанныеСтрокиСтарые, Null);
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
		...
		Пока ДанныеКонтекста.ЕстьИзменения Цикл
			...
			ДанныеКонтекста.Обработчики.ТекущиеИдентификаторы.Очистить();
...
//	Кэш -
//
Процедура глМодель_ИзменитьРеквизитСтроки(КонтекстИзменений, ДанныеСтроки, ИмяРеквизита, Значение, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
	...
	ИзмененныеРеквизиты = Новый Структура(ИмяРеквизита, ДанныеСтроки[ИмяРеквизита]);
	ДанныеСтроки[ИмяРеквизита] = Значение;
	глМодель_ПриИзмененииРеквизитовСтроки(КонтекстИзменений, ДанныеСтроки, ИзмененныеРеквизиты);
КонецПроцедуры
...
//	Кэш -
//
Процедура глМодель_ИзменитьРеквизитыСтроки(КонтекстИзменений, ДанныеСтроки, СтруктураЗначения, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
	...
	Для Каждого ЭлементСтруктуры Из СтруктураЗначения Цикл
		ИзмененныеРеквизиты.Вставить(ЭлементСтруктуры.Ключ, ДанныеСтроки[ЭлементСтруктуры.Ключ]);
...
//  ИменаРеквизитов	- Строка - имена измененных реквизитов через запятую
//					- Массив,ФиксированныйМассив - имена измененных реквизитов
//					- Соответствие,Структура - измененные реквизиты
//						* Ключ - имя
//						* Значение - старое значение
//	Кэш -
//
Процедура глМодель_ПриИзмененииРеквизитовСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРеквизитов, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
	...
	Пока ДанныеКонтекста.ЕстьИзменения Цикл
		...
		Если ДанныеКонтекста.ИзмененныеТаблицы <> Неопределено Тогда
			...
			СтарыеЗначения = СоответствиеСтроки[ДанныеСтроки];
			Если СтарыеЗначения = Неопределено Тогда
				СтарыеЗначения = Новый Структура;
				СоответствиеСтроки.Вставить(ДанныеСтроки, СтарыеЗначения);
			КонецЕсли;
			
			Если ТипЗнч(ИменаРеквизитов) = Тип("Структура")
				ИЛИ ТипЗнч(ИменаРеквизитов) = Тип("Соответствие") Тогда
				
				Для Каждого КлючИЗначение Из ИменаРеквизитов Цикл
					Если НЕ СтарыеЗначения.Свойство(КлючИЗначение.Ключ) Тогда
						СтарыеЗначения.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
					КонецЕсли;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
...
// 	КонтекстИзменений - Структура, ЛюбойОбъект - сушествующий контекст изменений
// 					- УправляемаяФорма - форма изменяемого объекта
//  СтрокаИмяКоллекции	- Строка - имя табличной части или добавленной таблицы
//						- СтрокаКоллекции - 
//  Кэш					 - 	 - 
// 
...
//   СтрокаКоллекции - 
//
Функция глМодель_ДобавитьСтроку(КонтекстИзменений, СтрокаИмяКоллекции, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
		НоваяСтрока = глМодель_ДобавитьСтроку(НовыйКонтекст, СтрокаИмяКоллекции);
		глМодель_ОбработатьДействия(НовыйКонтекст);
		Кэш = ДанныеКонтекста(НовыйКонтекст).Кэш;
		Возврат НоваяСтрока;
	КонецЕсли;
	
	Если ТипЗнч(СтрокаИмяКоллекции) = Тип("Строка") Тогда
		Если ДанныеКонтекста(КонтекстИзменений).ПараметрыМодели.ДобавленныеТаблицы.Получить(СтрокаИмяКоллекции) <> Неопределено Тогда
			НоваяСтрока = глМодель_Данные(КонтекстИзменений, СтрокаИмяКоллекции).Добавить();
		Иначе
			НоваяСтрока = глМодель_Данные(КонтекстИзменений)[СтрокаИмяКоллекции].Добавить();
		КонецЕсли;
	Иначе
		НоваяСтрока = СтрокаИмяКоллекции.Добавить();
	КонецЕсли;
	
	глМодель_ПриИзмененииРеквизитовСтроки(КонтекстИзменений, НоваяСтрока, "{_НоваяСтрока_}");
	Возврат НоваяСтрока;	
КонецФункции

// Отражение добавления строки постфактум
//
// Параметры:
// 	КонтекстИзменений - Структура, ЛюбойОбъект - сушествующий контекст изменений
// 					- УправляемаяФорма - форма изменяемого объекта
...
//  Кэш					 - 	 - 
//
Процедура глМодель_ПриДобавленииСтроки(КонтекстИзменений, ДанныеСтроки, Кэш = Неопределено) Экспорт
	Если НЕ глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Тогда
		НовыйКонтекст = ИнициализироватьКонтекстИзменений(КонтекстИзменений, , , Кэш);
		глМодель_ПриДобавленииСтроки(НовыйКонтекст, ДанныеСтроки);
		глМодель_ОбработатьДействия(НовыйКонтекст);
		Кэш = ДанныеКонтекста(НовыйКонтекст).Кэш;
		Возврат;
	КонецЕсли;
	
	глМодель_ПриИзмененииРеквизитовСтроки(КонтекстИзменений, ДанныеСтроки, "{_НоваяСтрока_}");
КонецПроцедуры
...
//
Функция глМодель_НачатьИзменения(Знач ФормаОбъект, ОбрабатыватьИзмененныеТаблицыПриЗавершении = Истина, ТолькоПросмотр = Ложь, Кэш = Неопределено) Экспорт
	Если ЭтоОбъектСсылочногоТипа(ФормаОбъект)
		И ФормаОбъект.ДополнительныеСвойства.Свойство("Модель_КонтекстИзменений") Тогда
		
		ДанныеКонтекста = ДанныеКонтекста(ФормаОбъект);
		Если НЕ ТолькоПросмотр Тогда
			Если НЕ ДанныеКонтекста.ТолькоПросмотр Тогда
				ВызватьИсключение "Попытка начала вложенных изменений";
			КонецЕсли;
			
			// перевод контекста в режим доступности изменений
			ДанныеКонтекста.ТолькоПросмотр = Ложь;
	...
	КонтекстИзменений = ИнициализироватьКонтекстИзменений(ФормаОбъект, , ТолькоПросмотр, Кэш);
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	
	Если НЕ ТолькоПросмотр
		И ОбрабатыватьИзмененныеТаблицыПриЗавершении Тогда
...
Функция глМодель_ПодготовитьДляЧтения(Знач Объект) Экспорт
	Возврат глМодель_НачатьИзменения(Объект, , Истина);
КонецФункции
...
//	Кэш -
//
Процедура глМодель_ЗавершитьИзменения(КонтекстИзменений, Кэш = Неопределено) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	Если ДанныеКонтекста.ТолькоПросмотр Тогда
		ВызватьИсключение "Попытка изменения контекста, открытого только для чтения";
	КонецЕсли;
	
	Если ДанныеКонтекста.ИзмененныеТаблицы <> Неопределено Тогда
		ОбработатьИзменениеТаблиц(КонтекстИзменений);
	КонецЕсли;
	
	глМодель_ОбработатьДействия(КонтекстИзменений);
	Кэш = ДанныеКонтекста(КонтекстИзменений).Кэш;
...
Функция глМодель_Модуль(Знач ОбъектФорма) Экспорт
	...
	ИначеЕсли ТипЗнч(ОбъектФорма) = Тип("Строка") Тогда
		ИмяОбъекта = ОбъектФорма;
	Иначе
		ИмяОбъекта = ОбъектФорма.Метаданные().ПолноеИмя();
	КонецЕсли;
	
	МассивИмяОбъекта = СтрРазделить(ИмяОбъекта, ".");
	Результат = Вычислить("Модель_" + МассивИмяОбъекта[0] + "_" + МассивИмяОбъекта[1]);
	Возврат Результат;	
КонецФункции
...
//					- Соответствие,Структура - коллекция, в ключах которой указаны имена реквизитов
// 
// Возвращаемое значение:
...
Функция глМодель_ЕстьРеквизиты(Данные, ИменаРеквизитов) Экспорт
	...
	Для Инд = 1 По МаксИнд Цикл
		...
		Если СтрНачинаетсяС(ИмяРеквизита, "[")
			И СтрЗаканчиваетсяНа(ИмяРеквизита, "]") Тогда
				
			МассивИменаРеквизитов.Удалить(МаксИнд - Инд);
			
		ИначеЕсли СтрНачинаетсяС(ИмяРеквизита, "{")
			И СтрЗаканчиваетсяНа(ИмяРеквизита, "}") Тогда
		...
		Иначе
			...
			Если Поз > 0 Тогда
				ИмяТаблицы = Лев(ИмяРеквизита, Поз - 1);
				СтруктураЗначения.Вставить(ИмяТаблицы, Null);
			Иначе
...
// Возвращает строку с именами существующих реквизитов
//
// Параметры:
//  Данные			 - ЛюбойОбъект,СтрокаКоллекции - 
//  ИменаРеквизитов	- Строка - имена реквизитов через запятую
...
//  Строка - 
//
Функция глМодель_СуществующиеРеквизиты(Данные, ИменаРеквизитов) Экспорт
	МассивИменаРеквизитов = глМодель_ПолучитьКлючи(ИменаРеквизитов);
	СтруктураЗначения = Новый Структура;
	МаксИнд = МассивИменаРеквизитов.Количество();
	Для Инд = 1 По МаксИнд Цикл
	...
	МаксИнд = МассивИменаРеквизитов.Количество();
	Для Инд = 1 По МаксИнд Цикл
		Если СтруктураЗначения[МассивИменаРеквизитов[МаксИнд - Инд]] = Null Тогда
			МассивИменаРеквизитов.Удалить(МаксИнд - Инд);
		КонецЕсли;
	КонецЦикла;
		
	Возврат СтрСоединить(МассивИменаРеквизитов, ",");
КонецФункции
...
//					- Массив,ФиксированныйМассив - имена измененных реквизитов
//					- Соответствие,Структура - коллекция, в ключах которой указаны имена измененных реквизитов
//  ИсключаемыеРеквизиты	- Строка - имена измененных реквизитов через запятую
//					- Массив,ФиксированныйМассив - имена измененных реквизитов
//					- Соответствие,Структура - коллекция, в ключах которой указаны имена измененных реквизитов
// 
// Возвращаемое значение:
...
//
Функция глМодель_ДанныеИзменены(ДанныеСтарые, ДанныеНовые, ИменаРеквизитов = Неопределено, ИсключаемыеРеквизиты = Неопределено) Экспорт
	...
	Если ИсключаемыеРеквизиты <> Неопределено Тогда
		МассивИсключаемые = глМодель_ПолучитьКлючи(ИсключаемыеРеквизиты);
	КонецЕсли;
	
	Если ИменаРеквизитов = Неопределено Тогда
		СтруктураЗначения = Новый Структура;
		Для Каждого ЭлементСтруктуры Из ДанныеСтарые Цикл
			Если ИсключаемыеРеквизиты <> Неопределено
				И МассивИсключаемые.Найти(ЭлементСтруктуры.Ключ) <> Неопределено Тогда
				
				Продолжить;
			КонецЕсли;
			
			СтруктураЗначения.Вставить(ЭлементСтруктуры.Ключ, Null);
		КонецЦикла;
		
		ЗаполнитьЗначенияСвойств(СтруктураЗначения, ДанныеНовые);
		Для Каждого ЭлементСтруктуры Из СтруктураЗначения Цикл
			Если ЭлементСтруктуры.Значение <> Null
				И ЭлементСтруктуры.Значение <> ДанныеСтарые[ЭлементСтруктуры.Ключ] Тогда
	...
	Иначе
		Для Каждого ИмяРеквизита Из глМодель_ПолучитьКлючи(ИменаРеквизитов) Цикл
			Если ИсключаемыеРеквизиты <> Неопределено
				И МассивИсключаемые.Найти(ИмяРеквизита) <> Неопределено Тогда
				
				Продолжить;
			КонецЕсли;
			
			Если ДанныеСтарые.Свойство(ИмяРеквизита)
				И ДанныеСтарые[ИмяРеквизита] <> ДанныеНовые[ИмяРеквизита] Тогда
...
// Устанавливает строки, которые были изменены в обработчике
//
// Параметры:
// 	КонтекстИзменений - Структура, ЛюбойОбъект - сушествующий контекст изменений
// 					- УправляемаяФорма - форма изменяемого объекта
//  ИмяТаблицы			 - Строка - 
//  ИзмененныеСтроки	 - Массив - данные измененных строк
//
Процедура глМодель_УстановитьИзмененныеСтроки(КонтекстИзменений, ИмяТаблицы, ИзмененныеСтроки = Неопределено) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	СтруктураОбработчиков = ДанныеКонтекста.Обработчики;
	
	ИзмененныеСтрокиРеквизита = СтруктураОбработчиков.ИзмененныеРеквизиты[ИмяТаблицы];
	Если ИзмененныеСтрокиРеквизита = Null Тогда
		ИзмененныеСтрокиРеквизита = Новый Соответствие;
		СтруктураОбработчиков.ИзмененныеРеквизиты[ИмяТаблицы] = ИзмененныеСтрокиРеквизита;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из ИзмененныеСтроки Цикл
		ИзмененныеСтрокиРеквизита.Вставить(СтрокаТаблицы);	
	КонецЦикла;
КонецПроцедуры
...
Процедура глМодель_ПриЧтенииСозданииНаСервере(Форма, ОбъектСсылка = Неопределено) Экспорт
	КонтекстИзменений = ИнициализироватьКонтекстИзменений(Форма, Истина);
	
	Если глМодель_ЕстьРеквизиты(Форма, "Модель_Параметры") Тогда
		ПараметрыМодели = Форма.Модель_Параметры;
	Иначе
		ПараметрыМодели = ДанныеКонтекста(КонтекстИзменений).ПараметрыМодели;
...
//  Параметры	- Структура - значения параметров обработчика
//						- Строка - имена параметров через запятую, которым будут установлено значение Истина
//						- Булево - если Истина, то параметры необходимо заполнять внутри вызова глМодель_НачатьУстановкуПараметровОбработчика
// 
// Возвращаемое значение:
//  Булево - 
//
Функция глМодель_ВыполнятьОбработчик(КонтекстИзменений, ИменаРезультатов, ИменаПараметров, Параметры = Ложь) Экспорт
	Возврат глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, Неопределено, ИменаРезультатов, ИменаПараметров, Параметры);
КонецФункции
...
//  Булево - 
//
Функция глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРезультатов, ИменаПараметров, Параметры = Ложь) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	ДанныеКонтекста.ДанныеТекущейСтроки = ДанныеСтроки;
	ДанныеКонтекста.ДействияОтмены = Неопределено;
	...
	ДанныеКонтекста.ИдентификаторТекущегоОбработчика = ИдентификаторОбработчика;
	Если ДанныеКонтекста.НомерЭтапа = 2 Тогда
		...
		Для Каждого ИмяПараметра Из глМодель_ПолучитьКлючи(ИменаПараметров) Цикл
			Если СтрНачинаетсяС(ИмяПараметра, "[")
				И СтрЗаканчиваетсяНа(ИмяПараметра, "]") Тогда
			...
			ИначеЕсли СтрНачинаетсяС(ИмяПараметра, "{")
				И СтрЗаканчиваетсяНа(ИмяПараметра, "}") Тогда
				
				// виртуальные реквизиты не учитываются
				
			ИначеЕсли СтруктураОбработчиков.Результаты[ИмяПараметра] <> Неопределено
				ИЛИ СтруктураОбработчиков.РеквизитыРезультатов[ИмяПараметра] <> Неопределено Тогда
		...
		ДанныеКонтекста.ЕстьИзменения = Истина;
		ДанныеКонтекста.ПараметрыТекущегоОбработчика = СтруктураОбработчиков.Идентификаторы[ИдентификаторОбработчика];
		
		СтруктураОбработчиков.Идентификаторы.Удалить(ИдентификаторОбработчика);
		Для Каждого ИмяРезультата Из глМодель_ПолучитьКлючи(ИменаРезультатов) Цикл
			...
			Поз = Найти(ИмяРезультата, ".");
			Если Поз > 0 Тогда
				ИмяТаблицы = Лев(ИмяРезультата, Поз - 1);
				ИмяРеквизита = Сред(ИмяРезультата, Поз + 1);
				
				РеквизитыТаблицы = СтруктураОбработчиков.РеквизитыРезультатов[ИмяТаблицы];
				РеквизитыТаблицы.Удалить(ИмяРеквизита);
				
				Если РеквизитыТаблицы.Количество() = 0 Тогда
					СтруктураОбработчиков.РеквизитыРезультатов.Удалить(ИмяТаблицы);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		
		Если ДанныеКонтекста.ИзмененныеТаблицы <> Неопределено
			...
			Для Каждого ИмяРезультата Из глМодель_ПолучитьКлючи(ИменаРезультатов) Цикл
				Если НЕ СтарыеЗначения.Свойство(ИмяРезультата) Тогда
					СтарыеЗначения.Вставить(ИмяРезультата, ДанныеСтроки[ИмяРезультата]);
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		...
		Возврат Истина;
	Иначе
		Если СтруктураОбработчиков.ТекущиеИдентификаторы[ИдентификаторОбработчика] <> Неопределено Тогда
			ВызватьИсключение СтрШаблон("Повторное определение обработчика с идентификатором %1", ИдентификаторОбработчика);
		Иначе
			СтруктураОбработчиков.ТекущиеИдентификаторы.Вставить(ИдентификаторОбработчика, Истина);
		КонецЕсли;
		...
		Для Каждого ИмяРезультата Из МассивИменаРезультата Цикл
			...
			Если Поз > 0 Тогда
				...
				ИзмененныеСтроки = СтруктураОбработчиков.ИзмененныеРеквизиты[ИмяТаблицы];
				Если ИзмененныеСтроки <> Неопределено Тогда
					Если ТипЗнч(ИзмененныеСтроки) = Тип("Соответствие") Тогда
						Для Каждого ЭлементСоответствия Из ИзмененныеСтроки Цикл
							Если ТипЗнч(ЭлементСоответствия.Значение) = Тип("Структура") Тогда
								Если ЭлементСоответствия.Значение.Свойство(ИмяРеквизита) Тогда
									Возврат Ложь;
								КонецЕсли;
							Иначе
								Возврат Ложь;
							КонецЕсли;
						КонецЦикла;
					ИначеЕсли ИзмененныеСтроки <> Null Тогда
						Возврат Ложь;
					КонецЕсли;
				КонецЕсли;
			Иначе
		...
		Если ТипЗнч(Параметры) = Тип("Структура") Тогда
			Для Каждого ЭлементСтруктуры Из Параметры Цикл
				ПроверитьИмяПараметраОбработчика(КонтекстИзменений, ЭлементСтруктуры.Ключ);
				ПараметрыОбработчика.Вставить(ЭлементСтруктуры.Ключ, ЭлементСтруктуры.Значение);
			КонецЦикла;
			
		ИначеЕсли ТипЗнч(Параметры) = Тип("Строка") Тогда
			Для Каждого ИмяПараметра Из глМодель_ПолучитьКлючи(Параметры) Цикл
				ПроверитьИмяПараметраОбработчика(КонтекстИзменений, ИмяПараметра);
				ПараметрыОбработчика.Вставить(ИмяПараметра, Истина);
			КонецЦикла;
		КонецЕсли;
		
		Если ПараметрыОбработчика.Свойство("_Отказ_")
			И ПараметрыОбработчика["_Отказ_"] Тогда
			
			Возврат Ложь;
		КонецЕсли;
		
		Если Параметры = Истина Тогда
			ДействияОтмены = Новый Массив;
		Иначе
			ДействияОтмены = Неопределено;
		КонецЕсли;
		...
		Для Каждого ИмяРезультата Из МассивИменаРезультата Цикл
			...
			Для Каждого ИмяПараметра Из МассивИменаПараметров Цикл
				...
				ВставитьСвойство(РезультатыПараметра, ИмяРезультата, Истина, ДействияОтмены);
				ВставитьСвойство(ПараметрыРезультата, ИмяПараметра, Истина, ДействияОтмены)
			КонецЦикла;
			
			Поз = Найти(ИмяРезультата, ".");
			ВставитьСвойство(СтруктураОбработчиков.ИзмененныеРеквизиты
				,?(Поз > 0, Лев(ИмяРезультата, Поз - 1), ИмяРезультата)
				,?(ДанныеСтроки = Неопределено, Null, ДанныеСтроки[ИмяРезультата])
				,ДействияОтмены);
		КонецЦикла;
		
		УстановитьСвойствоКонтекста(КонтекстИзменений, "ЕстьИзменения", Истина, ДействияОтмены);
		
		Для Каждого ИмяРезультата Из МассивИменаРезультата Цикл
			ВставитьСвойство(СтруктураОбработчиков.Параметры, ИмяРезультата, Истина, ДействияОтмены);
			ВставитьСвойство(СтруктураОбработчиков.Результаты, ИмяРезультата, Истина, ДействияОтмены);
			...
			Если Поз > 0 Тогда
				...
				ВставитьСвойство(СтруктураОбработчиков.Параметры, ИмяТаблицы, Истина, ДействияОтмены);
				ВставитьСвойство(СтруктураОбработчиков.РеквизитыРезультатов, ИмяТаблицы, Новый Структура(ИмяРеквизита, Истина), ДействияОтмены);
			КонецЕсли;
		КонецЦикла;
		
		ВставитьСвойство(СтруктураОбработчиков.Идентификаторы, ИдентификаторОбработчика, ПараметрыОбработчика, ДействияОтмены);
		
		Если ПараметрыОбработчика.Свойство("_Сервер_")
			И ПараметрыОбработчика["_Сервер_"] Тогда
			
			УстановитьСвойствоКонтекста(КонтекстИзменений, "ТребуетсяВызовСервера", Истина, ДействияОтмены);
		КонецЕсли;
		
		ДанныеКонтекста.ДействияОтмены = ДействияОтмены;
		Возврат Параметры = Истина;
	КонецЕсли;
КонецФункции

// Возвращает возможность установить параметры внутри обработчика
//
// Параметры:
//  КонтекстИзменений	 	- Структура, ЛюбойОбъект - контекст изменений
// 
...
//  Булево - 
//
Функция глМодель_НачатьУстановкуПараметровОбработчика(КонтекстИзменений) Экспорт
	Возврат ДанныеКонтекста(КонтекстИзменений).НомерЭтапа = 1;
КонецФункции

// Установка параметра текущего обработчика
//
// Параметры:
//  КонтекстИзменений	 - Структура, ЛюбойОбъект - контекст изменений
//  ИмяПараметра		 - Строка - произвольное имена параметра
//						 	* "_Отказ_" - отмена регистрации обработчика
//							* "_Сервер_" - для выполнения обработчика требуется вызов сервера
//  Значение			 - 	 - 
//
Процедура глМодель_УстановитьПараметрОбработчика(КонтекстИзменений, ИмяПараметра, Значение) Экспорт
	ПроверитьИмяПараметраОбработчика(КонтекстИзменений, ИмяПараметра);
	
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	Если ДанныеКонтекста.ДанныеТекущейСтроки <> Неопределено Тогда
		СтруктураОбработчиков = ДанныеКонтекста.ОбработчикиСтроки[ДанныеКонтекста.ДанныеТекущейСтроки]
	Иначе
		СтруктураОбработчиков = ДанныеКонтекста.Обработчики;
	КонецЕсли;
	
	СтруктураОбработчиков.Идентификаторы[ДанныеКонтекста.ИдентификаторТекущегоОбработчика].Вставить(ИмяПараметра, Значение);
	
	Если ИмяПараметра = "_Сервер_"
		И Значение = Истина Тогда
		
		ДанныеКонтекста.ТребуетсяВызовСервера = Истина;
		
	ИначеЕсли ИмяПараметра = "_Отказ_"
		И Значение = Истина Тогда
		
		Для Каждого ДействиеОтмены Из ДанныеКонтекста.ДействияОтмены Цикл
			Если ДействиеОтмены.Объект = "ДанныеКонтекста" Тогда
				ЗаполнитьЗначенияСвойств(ДанныеКонтекста, ДействиеОтмены.Параметр);
				
			ИначеЕсли ДействиеОтмены.Свойство("Свойство") Тогда
				ДействиеОтмены.Объект[ДействиеОтмены.Параметр].Удалить(ДействиеОтмены.Свойство);
				Если ДействиеОтмены.Объект[ДействиеОтмены.Параметр].Количество() = 0 Тогда
					ДействиеОтмены.Объект.Удалить(ДействиеОтмены.Параметр);
				КонецЕсли;
			Иначе
				ДействиеОтмены.Объект.Удалить(ДействиеОтмены.Параметр);
			КонецЕсли;
		...
		СтруктураОбработчиков.Идентификаторы.Удалить(ДанныеКонтекста.ИдентификаторТекущегоОбработчика);
	КонецЕсли;
КонецПроцедуры

// Получение параметра текущего обработчика
//
// Параметры:
//  КонтекстИзменений	 - Структура, ЛюбойОбъект - контекст изменений
//  ИмяПараметра		 - 	 - 
...
//   - 
//
Функция глМодель_ПолучитьПараметрОбработчика(КонтекстИзменений, ИмяПараметра) Экспорт
	Возврат ДанныеКонтекста(КонтекстИзменений).ПараметрыТекущегоОбработчика[ИмяПараметра];
КонецФункции

// Установка свойства контекста
//
// Параметры:
//  КонтекстИзменений	 	- Структура, ЛюбойОбъект - контекст изменений
//  ИмяПараметра		 - 	 - 
//  Значение			 - 	 - 
//
Процедура глМодель_УстановитьСвойствоКонтекста(КонтекстИзменений, ИмяПараметра, Значение) Экспорт
	Если ИмяПараметра = "Кэш" Тогда
		ДанныеКонтекста(КонтекстИзменений)[ИмяПараметра] = Значение;
	Иначе
		ВызватьИсключение "Недопустимый параметр для записи"
	КонецЕсли;
КонецПроцедуры

// Получение свойства контекста
//
// Параметры:
//  КонтекстИзменений	 	- Структура, ЛюбойОбъект - контекст изменений
//  ИмяПараметра		 - 	 - 
...
//   - 
//
Функция глМодель_ПолучитьСвойствоКонтекста(КонтекстИзменений, ИмяПараметра) Экспорт
	Если ИмяПараметра = "Кэш" Тогда
		Возврат ДанныеКонтекста(КонтекстИзменений)[ИмяПараметра];
	Иначе
		ВызватьИсключение "Недопустимый параметр для чтения"
	КонецЕсли;
КонецФункции
...
// 	ПроверятьЗначения - Булево - нужно проверять изменение значения реквизита, если оно известно
// Возвращаемое значение:
// 	Булево - 
Функция глМодель_РеквизитыИзменены(КонтекстИзменений, ИменаРеквизитов, ПроверятьЗначения = Ложь) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	Объект = глМодель_Данные(КонтекстИзменений);
	...
	Для Каждого ИмяРеквизита Из глМодель_ПолучитьКлючи(ИменаРеквизитов) Цикл
		...
		Если РезультатИзменения = Null
			ИЛИ РезультатИзменения <> Неопределено
				И НЕ ПроверятьЗначения
			ИЛИ РезультатИзменения <> Неопределено
				И Объект[ИмяРеквизита] <> РезультатИзменения Тогда
...
// Возвращаемое значение:
// 	Булево - 
Функция глМодель_РеквизитыСтрокиИзменены(КонтекстИзменений, ДанныеСтроки, ИменаРеквизитов, ПроверятьЗначения = Ложь) Экспорт
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	...
	Для Каждого ИмяРеквизита Из глМодель_ПолучитьКлючи(ИменаРеквизитов) Цикл
		...
		Если РезультатИзменения = Null
			ИЛИ РезультатИзменения <> Неопределено
				И НЕ ПроверятьЗначения
			ИЛИ РезультатИзменения <> Неопределено
				И ДанныеСтроки[ИмяРеквизита] <> РезультатИзменения Тогда
...
// 	ИмяТаблицы - Строка
// Возвращаемое значение:
//	Неопределено - невозможно выявить измененные строки
// 	Соответствие -
//		* Ключ - данные строки
//		* Значение - старые значения измененных реквизитов строки
Функция глМодель_ПолучитьИзмененныеСтроки(КонтекстИзменений, ИмяТаблицы) Экспорт
	...
	Если РезультатИзменения = Null Тогда
		Возврат Неопределено;
	ИначеЕсли РезультатИзменения = Неопределено Тогда
...
Функция глМодель_ТребуетсяУстановитьСвойства(КонтекстИзменений, ИменаСвойств, ТребуемыеСвойства) Экспорт
	...
	ДанныеКонтекста.ДанныеТекущейСтроки = Неопределено;
...
Процедура глМодель_УстановитьЗначениеСвойства(КонтекстИзменений, Значение) Экспорт
	...
	Если ДанныеКонтекста.ДанныеТекущейСтроки <> Неопределено Тогда
		глМодель_УстановитьСвойствоСтроки(КонтекстИзменений
			,ДанныеКонтекста.ДанныеТекущейСтроки, ДанныеКонтекста.ИмяТекущегоСвойства, Значение);
	Иначе
		глМодель_УстановитьСвойство(КонтекстИзменений, ДанныеКонтекста.ИмяТекущегоСвойства, Значение);
	КонецЕсли
КонецПроцедуры

// Проверяет, используется ли в переданном контексте изменений отложенное изменение таблиц
//
// Параметры:
// 	КонтекстИзменений - Структура, ЛюбойОбъект - контекст измненений
// 
...
// Проверяет, является ли переданный объект контекстом изменений
//
// Параметры:
// 	КонтекстИзменений - Структура, ЛюбойОбъект - контекст измненений
// 
...
//  Булево - 
//
Функция глМодель_ЭтоКонтекстИзменений(КонтекстИзменений) Экспорт
	Возврат ТипЗнч(КонтекстИзменений) = Тип("Структура")
		ИЛИ ЭтоОбъектСсылочногоТипа(КонтекстИзменений)
			И КонтекстИзменений.ДополнительныеСвойства.Свойство("Модель_КонтекстИзменений");
КонецФункции
#КонецОбласти

Функция ИнициализироватьКонтекстИзменений(ОбъектФорма, ИнициализацияФормы = Ложь, ТолькоПросмотр = Ложь, Кэш = Неопределено, ДанныеИсходнойСтроки = Неопределено)
	...
	КонтекстИзменений.Вставить("ДанныеТекущейСтроки");
	...
	КонтекстИзменений.Вставить("ТолькоПросмотр", ТолькоПросмотр);
	КонтекстИзменений.Вставить("Кэш", Кэш);
	КонтекстИзменений.Вставить("ИдентификаторТекущегоОбработчика");
	КонтекстИзменений.Вставить("ДействияОтмены");
	КонтекстИзменений.Вставить("ПараметрыТекущегоОбработчика");
	КонтекстИзменений.Вставить("СлужебныеПараметрыОбработчика", Новый Структура("_Сервер_,_Отказ_"));
...
Функция ПолучитьСтруктуруОбработчиков()
	Возврат Новый Структура("Результаты,РеквизитыРезультатов,Параметры,ЗависимостиПрямые,ЗависимостиОбратные,Свойства,СвойстваКОбновлению,Идентификаторы,ТекущиеИдентификаторы,ИзмененныеРеквизиты"
			,Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие, Новый Соответствие);
КонецФункции
...
Функция ПолучитьДанныеСтроки(КонтекстИзменений, Ключ)
	Если ДанныеКонтекста(КонтекстИзменений).ПараметрыМодели.ДобавленныеТаблицы.Получить(Ключ.ИмяТаблицы) <> Неопределено Тогда
		Возврат глМодель_Данные(КонтекстИзменений, Ключ.ИмяТаблицы).НайтиПоИдентификатору(Ключ.ИдентификаторСтроки);
	Иначе
		Возврат глМодель_Данные(КонтекстИзменений)[Ключ.ИмяТаблицы].НайтиПоИдентификатору(Ключ.ИдентификаторСтроки);
	КонецЕсли;
КонецФункции
...
Процедура СохранитьДанныеПродолжения(КонтекстИзменений, ДанныеКонтекста)
	...
	Если ДанныеКонтекста.ИзмененныеТаблицы <> Неопределено Тогда
		...
		Для Каждого ЭлементСоответствия Из ДанныеКонтекста.ИзмененныеТаблицы Цикл
			...
			НовоеСоответствие.Вставить(ЭлементСоответствия.Ключ, Результат);
		КонецЦикла;
		
		ДанныеКонтекста.ИзмененныеТаблицы = НовоеСоответствие;
	КонецЕсли;
	...
	Для Каждого ЭлементСоответствия Из ДанныеКонтекста.Обработчики.ИзмененныеРеквизиты Цикл
		Если ТипЗнч(ЭлементСоответствия.Значение) = Тип("Соответствие") Тогда
			...
			НовоеСоответствие.Вставить(ЭлементСоответствия.Ключ, Результат);
		Иначе
			НовоеСоответствие.Вставить(ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение);
		КонецЕсли;
	КонецЦикла;
	
	ДанныеКонтекста.Обработчики.ИзмененныеРеквизиты = НовоеСоответствие;
...
Процедура ВставитьСвойство(Коллекция, Ключ, Значение, ДанныеОтмены = Неопределено)
	Если ТипЗнч(Значение) = Тип("Структура") Тогда
		РеквизитыОтмены = Новый Массив;
		
		ТекущееЗначение = Коллекция[Ключ];
		Если ТекущееЗначение = Неопределено Тогда
			Коллекция.Вставить(Ключ, Значение);
			
			Если ДанныеОтмены <> Неопределено Тогда
				Для Каждого ЭлементСтруктуры Из Значение Цикл
					ДанныеОтмены.Добавить(Новый Структура("Объект,Параметр,Свойство", Коллекция, Ключ, ЭлементСтруктуры.Ключ));
				КонецЦикла;
			КонецЕсли;;
		Иначе
			Для Каждого ЭлементСтруктуры Из Значение Цикл
				Если ДанныеОтмены <> Неопределено
					И ТекущееЗначение.Свойство(ЭлементСтруктуры.Ключ) = Неопределено Тогда
					
					ДанныеОтмены.Добавить(Новый Структура("Объект,Параметр,Свойство", Коллекция, Ключ, ЭлементСтруктуры.Ключ));
				КонецЕсли;
				
				ТекущееЗначение.Вставить(ЭлементСтруктуры.Ключ, ЭлементСтруктуры.Значение);	
			КонецЦикла;
		КонецЕсли;
	Иначе
		Если ДанныеОтмены <> Неопределено
			И Коллекция[Ключ] = Неопределено Тогда
			
			ДанныеОтмены.Добавить(Новый Структура("Объект,Параметр", Коллекция, Ключ));
		КонецЕсли;
...
Процедура УстановитьСвойствоКонтекста(КонтекстИзменений, Ключ, Значение, ДанныеОтмены = Неопределено)
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	Если ДанныеОтмены <> Неопределено Тогда
		Если ДанныеКонтекста[Ключ] <> Значение Тогда
			ДанныеОтмены.Добавить(Новый Структура("Объект,Параметр", "ДанныеКонтекста", Новый Структура(Ключ, ДанныеКонтекста[Ключ])));
		КонецЕсли;
	КонецЕсли;
	
	ДанныеКонтекста[Ключ] = Значение;
КонецПроцедуры

Процедура ПроверитьИмяПараметраОбработчика(КонтекстИзменений, ИмяПараметра)
	Если СтрНачинаетсяС(ИмяПараметра, "_")
		И СтрЗаканчиваетсяНа(ИмяПараметра, "_")
		И НЕ ДанныеКонтекста(КонтекстИзменений).СлужебныеПараметрыОбработчика.Свойство(ИмяПараметра) Тогда
		
		ВызватьИсключение СтрШаблон("Недопустимое имя служебного параметра обработчика %1", ИмяПараметра);
	КонецЕсли;
КонецПроцедуры

Процедура ОбработатьИзменениеТаблиц(КонтекстИзменений)
	ДанныеКонтекста = ДанныеКонтекста(КонтекстИзменений);
	ДанныеКонтекста.ОтложенноеИзменениеТаблиц = Ложь;
	
	Если ДанныеКонтекста.ИзмененныеТаблицы.Количество() > 1 Тогда
		ИменаТаблиц = Новый Массив;
		ДанныеТаблиц = Новый Структура;
		Для Каждого ЭлементСоответствия Из ДанныеКонтекста.ИзмененныеТаблицы Цикл
			ИменаТаблиц.Добавить(ЭлементСоответствия.Ключ);
			ДанныеТаблиц.Вставить(ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение);
		КонецЦикла;
		
		глМодель_ПриИзмененииРеквизитов(КонтекстИзменений, СтрСоединить(ИменаТаблиц, ","), ДанныеТаблиц);
	Иначе
		Для Каждого ЭлементСоответствия Из ДанныеКонтекста.ИзмененныеТаблицы Цикл
			глМодель_ПриИзмененииРеквизитов(КонтекстИзменений, ЭлементСоответствия.Ключ, ЭлементСоответствия.Значение);
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьПараметрыМодели(ОбъектФорма) Экспорт
	...
	МетаданныеМодуля = Метаданные.ОбщиеМодули["Модель_" + СтрЗаменить(СтруктураРезультат.ИмяОбъекта, ".", "_")];
