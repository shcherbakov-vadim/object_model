...
Процедура глМодель_ПриИзмененииРеквизитов(КонтекстИзменений, ИменаРеквизитов, ДанныеСтрокиСтарые = Неопределено, ДанныеСтрокиНовые = Неопределено) Экспорт
	...
	Иначе
		...
		ДанныеКонтекста.ВыполнятьОбработчики = Ложь;
		ДанныеКонтекста.ЕстьИзменения = Истина;
		
		ТекДанныеСтрокиСтарые = ДанныеСтрокиСтарые;
		ТекДанныеСтрокиНовые = ДанныеСтрокиНовые;
		Пока ДанныеКонтекста.ЕстьИзменения Цикл
			...
			глМодель_Модуль(КонтекстИзменений).ПриВыполненииОбработчиков(КонтекстИзменений, ТекДанныеСтрокиСтарые, ТекДанныеСтрокиНовые);
		
			ТекДанныеСтрокиСтарые = Неопределено;
			ТекДанныеСтрокиНовые = Неопределено;
		КонецЦикла;
	КонецЕсли;
	...
	ДанныеКонтекста.ВыполнятьОбработчики = Истина;
	ДанныеКонтекста.ЕстьИзменения = Истина;
...
Процедура глМодель_ПриИзмененииРеквизитовСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРеквизитов) Экспорт
	...
	Иначе
		...
		Пока ДанныеКонтекста.ЕстьИзменения Цикл
			ДанныеКонтекста.ЕстьИзменения = Ложь;
			Для Каждого ЭлементСоответствия Из ДанныеКонтекста.ОбработчикиСтроки Цикл
	...
	Пока ДанныеКонтекста.ЕстьИзменения Цикл
		...
		Если НЕ ДанныеКонтекста.ЕстьИзменения Тогда
			Для Каждого ДанныеОбработчикаСтроки Из ДанныеКонтекста.ОбработчикиСтроки Цикл
		...
		Если ДанныеКонтекста.ИзмененныеТаблицы <> Неопределено Тогда
			...
			ДанныеКонтекста.ИзмененныеТаблицы.Вставить(ИмяТаблицы, Истина);
		КонецЕсли;
	КонецЦикла;
...
Функция глМодель_Данные(Знач ОбъектФорма, ИмяРеквизита = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектФорма) = Тип("УправляемаяФорма") Тогда
		ВладелецДанных = ОбъектФорма.Объект;
	Иначе
		ВладелецДанных = ОбъектФорма;
	КонецЕсли;
	
	Если ИмяРеквизита <> Неопределено Тогда
		Результат = ПолучитьЗначениеРеквизита(ВладелецДанных, ИмяРеквизита);
	Иначе
		Результат = ВладелецДанных;
	КонецЕсли;

	Если ТипЗнч(Результат) = Тип("ДанныеФормыДерево") Тогда
...
// 	Имя - Строка - (<имя существующей таблицы>.)<имя реквизита>
...
Функция глМодель_ДобавитьРеквизит(Знач КонтекстРодителя, Имя, ТипЗначения, Хранение = "Нет") Экспорт
	Поз = Найти(Имя, ".");
	Если Поз > 0 Тогда
		ИмяТаблицы = Лев(Имя, Поз - 1);
		ИмяРеквизита = Сред(Имя, Поз + 1);
	Иначе
		ИмяТаблицы = "";
		ИмяРеквизита = Имя;
	КонецЕсли;
	...
	Иначе
		...
		Если ЗначениеЗаполнено(ИмяТаблицы) Тогда
			ДобавленныеРеквизиты = КонтекстРодителя.ПараметрыМодели.ДобавленныеРеквизитыСтроки[ИмяТаблицы];
			Если ДобавленныеРеквизиты = Неопределено Тогда
				ДобавленныеРеквизиты = Новый Структура;
				КонтекстРодителя.ПараметрыМодели.ДобавленныеРеквизитыСтроки.Вставить(ИмяТаблицы, ДобавленныеРеквизиты);
			КонецЕсли;
...
Функция глМодель_ПолучитьДобавленныеРеквизиты(КонтекстИзменений, Путь) Экспорт
	...
	Если МассивПуть[0] = "Объект" Тогда
		ТекущийРодитель = ПараметрыМодели.ДобавленныеРеквизитыСтроки[МассивПуть[1]];
...
Функция глМодель_ВыполнятьДействиеСтроки(КонтекстИзменений, ДанныеСтроки, ИмяДействия, ИменаПараметров) Экспорт
	...
	Если ДанныеКонтекста.ВыполнятьОбработчики Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, ИменаРезультатов, ИменаПараметров, ТребуетсяВызовСервера = Ложь) Экспорт
	...
	Возврат ДанныеКонтекста.ВыполнятьОбработчики;
КонецФункции
...
Процедура глМодель_ОбновлятьСвойстваСтроки(КонтекстИзменений, ДанныеСтроки, ИменаСвойств, ИменаПараметров) Экспорт
	...
	Если НЕ ДанныеКонтекста.ВыполнятьОбработчики Тогда
...
Функция глМодель_ТребуетсяУстановитьСвойства(КонтекстИзменений, ИменаСвойств, ТребуемыеСвойства) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ТребуемыеСвойства) Тогда
		Возврат Истина;
	Иначе
		МассивТребуемыеСвойства = ПолучитьКлючи(ТребуемыеСвойства);
		Для Каждого ИмяСвойства Из ПолучитьКлючи(ИменаСвойств) Цикл
...
Функция глМодель_ОбработатьДействия(КонтекстИзменений, Форма = Неопределено) Экспорт
	...
	Если Форма <> Неопределено Тогда
		ДанныеКонтекста.ОбъектФорма = Неопределено;
		ДанныеКонтекста.ПараметрыСтроки = Неопределено;
		ДанныеКонтекста.ОбработчикиСтроки = Неопределено;
		ДанныеКонтекста.ДанныеСтрокиТекущегоСвойства = Неопределено;
	КонецЕсли;
КонецФункции
...
Процедура СоздатьРеквизиты(КонтекстИзменений, Родитель = Неопределено, Путь = "")
	...
	Если Родитель = Неопределено Тогда
		Если ТипЗнч(ДанныеКонтекста.ОбъектФорма) = Тип("УправляемаяФорма") Тогда
			...
			Для Каждого ЭлементСоответствия Из ПараметрыМодели.ДобавленныеРеквизитыСтроки Цикл
...
Функция ПолучитьПараметрыМодели(ОбъектФорма) Экспорт
	СтруктураРезультат = Новый Структура("Опции,ИменаТаблиц,ДобавленныеРеквизиты,ДобавленныеРеквизитыСтроки,ЕстьРеквизитыВместе,ЕстьРеквизитыОтдельно,ИмяОбъекта"
		,Новый Структура, Новый Массив, Новый Структура, Новый Соответствие, Ложь, Ложь, "");
	...
	Для Каждого Описание Из ПолучитьКлючи(МетаданныеМодуля.Комментарий) Цикл
