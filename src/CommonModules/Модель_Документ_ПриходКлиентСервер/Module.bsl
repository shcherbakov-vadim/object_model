...
Процедура ПриВыполненииОбработчиков(КонтекстИзменений, ДанныеСтрокиСтарые = Неопределено, ДанныеСтрокиНовые = Неопределено) Экспорт
	глМодель_ВыполнятьДействие(КонтекстИзменений, "ОбновитьИтоги", "Товары");
КонецПроцедуры
...
Процедура ПриВыполненииОбработчиковСтроки(КонтекстИзменений, ДанныеСтроки) Экспорт
	...
	Если глМодель_ВыполнятьОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, "Цена", "Номенклатура") Тогда
		ДанныеСтроки.Цена = ЦенообразованиеВызовСервера.ПолучитьЦенуНоменклатуры(ДанныеСтроки.Номенклатура, глМодель_Данные(КонтекстИзменений).Дата);
	КонецЕсли;
