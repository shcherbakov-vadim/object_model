...
- возможность добавлять на управляемую форму только те дополнительные реквизиты, которые необходимы для отображения
...
- **Модуль модели** - общий модуль имеющий следующий формат имени: Модуль_<тип объекта метаданных>_<вид объекта метаданных>КлиентСервер, у которого установлены флаги Клиент (управляемое приложение), Сервер, ВнешнееСоединение и Клиент (обычное приложение). В комментарии общего модуля можно указать опции.
...
```bsl
Если глМодель_ВыполняетсяОбработчик(КонтекстИзменений, <результирующие поля>, <исходные поля>) Тогда
...
```bsl
Если глМодель_ВыполняетсяОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, <результирующие поля>, <исходные поля>) Тогда
  <реализация обработчика>
КонецЕсли;
```
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	глМодель_ПриЗаписиНаСервере(ЭтаФорма, ТекущийОбъект);
КонецПроцедуры
'''
