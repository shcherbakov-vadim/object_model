...
```bsl
Если глМодель_ВыполняетсяОбработчикСтроки(КонтекстИзменений, ДанныеСтроки, "Поставщик", "Номенклатура,{Склад}") Тогда
...
```bsl
Если глМодель_ВыполняетсяОбработчик(КонтекстИзменений, "Склад") Тогда
